...
export default class GoodController {
  ...
  public async catalog({ request, response, view, session }: HttpContextContract) {
    try {
      const all = request.all()
      const catalog = await Database.from('land_goods_catalog').select('*').where({ level: 1, status: 1 }).orderBy('sort', 'asc').forPage(request.input('page', 1), 8)
      for (let index = 0; index < catalog.length; index++) {
        ...
        for (let log = 0; log < catalog[index].sub_catalog.length; log++) {
          catalog[index].sub_catalog[log].number = (await Database.from('land_goods').where({ good_catalog: catalog[index].sub_catalog[log].id, status: 1 }).count('* as total'))[0].total || 0
        }
      }
  ...
  public async show({ params, request, view, response }: HttpContextContract) {
    try {
      const all = request.all()
      const good = await Database.from('land_goods').where({ id: params.id, status: 1 }).first() || {}
